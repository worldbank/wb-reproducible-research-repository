...
If you are in Cowork or another setting with file access, act on the real project folder in Phase 3 — or, if the Phase 2 pathway decision was "extract to a new folder," act on the new package folder, copying in files rather than editing the original project. In chat without file access, Phase 3 produces the deliverables (README, main script, restructure plan, flag-fix list) for the user to place themselves.
...
0. Pathway decision. Ask the user which pathway applies, unless it is already obvious from the audit (e.g., the project folder already is a package with no unrelated content):
   - **In-place**: the project folder already approximately is the package (data/code/outputs mostly relevant, layout close to standard). Build by editing and reorganizing within it.
   - **Extract to a new folder**: the project folder is a working directory with substantial content unrelated to the manuscript (other papers, drafts, superseded analyses). Build a new, separate reproducibility-package folder and copy in only the minimum in-scope files identified in Phase 1 step 2, leaving the original project untouched. This is the default recommendation whenever the audit found material out of scope for the manuscript's exhibits.
   State which pathway applies and why before producing the artifacts below; the gap table and every Phase 3 action are written against whichever pathway is chosen (moves within the project vs. copies into the new folder).
...
1. Target structure. Whenever possible the package uses the standard three-folder tree at the top level:
   ...
   Show where each existing file maps into this tree and where new files belong. Use the project's existing conventions where they already meet the standard; do not impose renames the standard does not require — but do propose the three-folder tree when the current layout scatters data, code, and outputs together. If the pathway decision (step 0) is "extract to a new folder," this tree is rooted at the new package folder, not the original project — state the new folder's path and show the mapping as source-in-project → destination-in-new-folder for each in-scope file.
...
3. Gap table. One row per checklist gap and one per flag from Phase 1. Columns: item/flag, required, present now, action to close. Actions are concrete: "add DAS entry for OECD_self_employment.xlsx with URL and access year", "move the three hardcoded paths in 2_event_study.do into the path global in main.do", "add set seed 12345 before bootstrap in 04_inference.do". If the pathway is "extract to a new folder," phrase file actions as copies into the new folder ("copy 2_event_study.do into code/02_analysis/ in the new package, fixing the three hardcoded paths on the way in") rather than in-place moves. This table is the build plan.
...
If the Phase 2 pathway is "extract to a new folder," create that folder first and copy in-scope files into it per the gap table's mapping; the original project is not edited. If the pathway is "in-place," edit and reorganize the project folder directly. Either way, work through the approved gap table row by row.
